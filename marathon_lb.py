...
def has_group(groups, app_groups):
    ...
    if (len(frozenset(app_groups) & groups)):
        logger.debug("Mannnnnnn.... %s--->%s" %(groups,app_groups) )
        return True
    ...
    for app in sorted(apps, key=attrgetter('appId', 'servicePort')):
       ...
       #     continue
        ...
        for backend_service_idx, backendServer\
            ...
            logger.debug(
                "nnnnnnnn backend server %s:%d on %s",
                backendServer.ip,
                backendServer.port,
                backendServer.host)
...
def compareWriteAndReloadConfig(config, config_file, domain_map_array,
    ...
    logger.debug("kkkkkk.... %si---- %s" %(changed,config_valid))
...
def get_apps(marathon, apps=[]):
    ...
    for app in apps:
        ...
        if 'HAPROXY_DEPLOYMENT_GROUP' in app['labels']:
            ...
            logger.debug("manju..... %s" %(app['labels']))
            ...
            continue
        logger.debug("manju888...... %s" %(deployment_group))
        if deployment_group in deployment_groups:
            ...
            old = new = None
            logger.debug("manju4444...new %s ---> old %s" %(cur_date,prev_date))
        ...
        logger.debug("Manju9999... %s" %(deployment_groups))
    ...
    for app in processed_apps:
        ...
        logger.debug("Manju9999.... %s" %(marathon_app))
...
def make_config_valid_and_regenerate(marathon,
    ...
    try:
        ...
        logger.debug("mnaju99999... %s" %(valid_apps))
        if len(valid_apps) > 0:
